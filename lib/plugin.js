...
/**
 * Monkey-patches exec to perform deep calculation.
...
Query.prototype.exec = function (op, cb) {
  ...
  _exec.call(this, op, function (err, docs) {
    ...
    else execute(model, docs, paths, options, defaultOptions, promise.resolve.bind(promise))
  })
...
function deepPopulatePlugin(schema, defaultOptions) {
  ...
  function deepPopulate(model, docs, paths, options, cb) {
    ...
    execute(model, docs, paths, options, defaultOptions, cb)
  }
}

function execute(model, docs, paths, options, defaultOptions, cb) {
...
function createParams(model, docs, paths, options) {
  paths = isArray(paths) ? paths : (paths || '').split(/[\s,]+/) // space/comma delimited
